...
class template {
    ...
    private function replaceDBTags( $estigma, $cacheId,$mod ){
        ...
        if(!$mod):$block="Access denied";endif;
        // delete <!-- --> tags
    ...
    public function parseCss(){
        for($i=0;$i<(count($this->page->getCss()));$i++):
            ...
            $newContent = str_replace('<head>', '<head>'.'<link rel="stylesheet" type="text/css" href="http://'.$_SERVER['HTTP_HOST'].'/'.WEB_DIR.$css[$i].'" media="screen" />'.PHP_EOL, $this->page->getContent());
            $this->page->setContent( $newContent );
        endfor;
    }
    ...
    public function parseJs(){
        for($i=0;$i<(count($this->page->getJs()));$i++):
            $js = $this->page->getJs();
            $newContent = str_replace('<head>', '<head>'.'<script type="text/javascript" src="http://'.$_SERVER['HTTP_HOST'].'/'.WEB_DIR.$js[$i].'"></script>'.PHP_EOL, $this->page->getContent());
            $this->page->setContent( $newContent );
        endfor;
    }
