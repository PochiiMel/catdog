<?php
	...
	function paginar($numeroRegistros,$url){
		...
		$paginacion_str = '<ul class="pagination">';
		
		/* genera una cadea con html para establecer los enlaces de la paginacion */	
		if($pagina>1){
			$paginacion_str.= "<li><a href='{$url}pag=".($pagina-1)."'>";
		...
		for($i=$inicio;$i<=$final;$i++){
			if($i==$pagina)
			{
				$paginacion_str.= "<li><a href='javascript:void(0);' style='color:#000;'>".$i."</a></li>";
			}else{
